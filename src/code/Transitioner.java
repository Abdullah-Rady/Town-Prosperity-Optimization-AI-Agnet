...
public class Transitioner {
        ...
        public int calculateHeuristic(int actionValue, TownConstants constants){
            if(firstHeuristic) return firstHeuristicFunction(actionValue , constants);
        ...
        private int firstHeuristicFunction(int actionValue , TownConstants constants){
            if(actionValue == 1)
             ...
             if(actionValue == 4)
                return -3;
            if(actionValue == 5)
                return -constants.energyUseBUILD1-constants.foodUseBUILD1-constants.materialsUseBUILD1+ constants.prosperityBUILD1;


                return -constants.energyUseBUILD2-constants.foodUseBUILD2-constants.materialsUseBUILD2 + constants.prosperityBUILD2;


        }

    private int secondHeuristicFunction(int actionValue , TownConstants constants){
            // everything here is in negative as we want least price so we maximize (- price)
        if(actionValue == 1)
